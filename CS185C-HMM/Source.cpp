...
void getRoc(float* arr, unsigned int size, char* dset1, size_t len1, char* dset2, size_t len2, char* hmm_fp, size_t len3) {
	...
	factory << std::string(dset1) << "_stats.csv"; std::string mapper_file1 = factory.str();
	factory.str("");
	factory << std::string(dset2) << "_stats.csv"; std::string mapper_file2 = factory.str();
	...
	DataMapper mapper1 = generateDataMapFromStats(dataset_file1, tester.getM());
	tester.setDataMapper(mapper1);
	NewLineSeperatedLoader loader1 = NewLineSeperatedLoader(dataset_file1);

	DataMapper mapper2 = generateDataMapFromStats(dataset_file2, 0xffffffffU);
